...
(function() {
  ...
  var answerRaw = ae.textContent.trim();
  var rawHTML = ce.innerHTML;
  
  // 1. 正解データのパース
  // 通常: "c" や "a,c"
  // 連問: "F063: a / F064: e / F065: c" 
  var correctMap = {}; // { blockName: { 'a': true, 'c': true } }
  var defaultBlock = 'MAIN';
  
  if (answerRaw.indexOf(':') !== -1 && answerRaw.indexOf('/') !== -1) {
    // 連問形式
    var parts = answerRaw.split('/');
    parts.forEach(function(p) {
      ...
      if (colonIdx !== -1) {
        ...
        var vals = p.substring(colonIdx + 1).trim();
        var sel = {};
        vals.toLowerCase().split(',').forEach(function(v) {
          var t = v.trim(); if(t) sel[t] = true;
        });
        correctMap[key] = sel;
      }
    });
    ...
    // 通常形式
    var sel = {};
    answerRaw.toLowerCase().split(',').forEach(function(v) {
      var t = v.trim(); if(t) sel[t] = true;
    });
    correctMap[defaultBlock] = sel;
  }
  
  // 2. HTMLのパースとブロック分割
  var lines = rawHTML.split(/<br\s*\/?>/i).map(function(l) { return l.trim(); }).filter(function(l) { return l.length > 0; });
  
  var blocks = [];
  var currentBlock = { name: defaultBlock, lines: [] };
  
  lines.forEach(function(line) {
    // ── [F063] ── のような区切り文字列かをチェック
    var headerMatch = line.match(/^[\u2500\u2014\-]+\s*\[(.*?)\]\s*[\u2500\u2014\-]+$/);
    if (!headerMatch) {
       // 全角ハイフン・罫線などのバリエーション
       headerMatch = line.match(/^.*?[─-]{2,}\s*\[(.*?)\]\s*[─-]{2,}.*?$/);
    }
  ...
  if (blocks.length === 0 || Object.keys(correctMap).length === 0) {
  ...
  var allBlocksOk = true; // 1つでも間違えたら全体として不正解扱いとする
  
  blocks.forEach(function(blk, index) {
    var bName = blk.name;
    var bCorrect = correctMap[bName]; 
    
    // JSON側の連問ID（F063など）とヘッダの[F063]が完全に一致しないケースのフォールバック
    if (!bCorrect) {
       // 近似マッチ（番号部分など）を探すか、順番でフォールバック
       var ansKeys = Object.keys(correctMap);
       if (ansKeys[index]) {
         bCorrect = correctMap[ansKeys[index]];
       } else {
         bCorrect = {}; // セーフガード
       }
    }
    
    var isMulti = Object.keys(bCorrect).length > 1;
    
    var blockWrapper = document.createElement('div');
    blockWrapper.className = 'question-block';
    blockWrapper.style.marginBottom = '20px';
    
    // ヘッダー（連問の場合のみ表示）
    if (bName !== defaultBlock) {
    ...
    blk.lines.forEach(function(line) {
      ...
      var textObj = tempDiv.textContent || tempDiv.innerText || "";
      var cleanedText = textObj.trim().replace(/^[\s\(\[（【・]+/, '');
      var normLine = cleanedText.replace(/[ａ-ｅＡ-Ｅ]/g, function(s) { 
          return String.fromCharCode(s.charCodeAt(0) - 65248); 
      });
      var m = normLine.match(/^([a-e])/i);
        ...
        if (isMulti) {
          ...
          if (bCorrect[letter]) {
            ...
            btns.forEach(function(b) {
              if (bCorrect[b.getAttribute('data-letter')]) b.classList.add('correct');
            });
          }
          lockBlock();
          checkAllDone();
    ...
    if (isMulti) {
      ...
      cfm.addEventListener('click', function(e) {
        e.stopPropagation(); if (blockDone) return; blockDone = true;
        var sel = {};
        btns.forEach(function(b) {
          if (b.classList.contains('selected')) sel[b.getAttribute('data-letter')] = true;
        });
        
        var ok = Object.keys(bCorrect).length === Object.keys(sel).length;
        if (ok) Object.keys(bCorrect).forEach(function(k) { if (!sel[k]) ok = false; });
        
        btns.forEach(function(b) {
          var l = b.getAttribute('data-letter');
          if (bCorrect[l]) b.classList.add('correct');
        ...
        var h = blockWrapper.querySelector('.multi-hint'); if (h) h.style.display = 'none';
        checkAllDone();
      });
      blockWrapper.appendChild(cfm);
    }
    ...
    function checkAllDone() {
      allBlocksDone++;
      if (allBlocksDone === totalBlocks) {
        showGlobalBanner(allBlocksOk);
      }
    }
    
    area.appendChild(blockWrapper);
  });
  
  function showGlobalBanner(ok) {
    var acc = document.getElementById('raw-accuracy');
    var at = acc ? acc.textContent.trim() : '';
    // 連問の場合は正答率も "F063: 97.0% / F064: 90.0%" のようになっているのでそのまま表示
    var ah = at ? '<div class="banner-accuracy">\u6b63\u7b54\u7387:<br>' + at.replace(/\//g, '<br>') + '</div>' : '';
    banner.innerHTML = ok
      ? '<div class="banner-correct">\u2b55 \u5168\u554f\u6b63\u89e3\uff01</div>' + ah
      : '<div class="banner-wrong">\u274c \u4e0d\u6b63\u89e3\u3042\u308a</div>' + ah;
...
def convert_to_anki(
    ...
    for i, q in enumerate(questions):
        ...
        correct_answer_raw = result.get("correctAnswer", "")
        # 正解値を全角→半角変換し、a-eラベルに統一
        def normalize_answer(ans):
            ...
            for item in items:
                # 全角→半角変換
                item = item.replace('ａ','a').replace('ｂ','b').replace('ｃ','c').replace('ｄ','d').replace('ｅ','e')
                # 小文字化
                item = item.lower()
                # a-e以外は無視
        ...
        correct_answer = escape_html(normalize_answer(correct_answer_raw))
...
""",
    ...
    # ── 複数フォルダの一括処理かどうかの判定 ──
    # 特定のフォルダを指定しているか（フォルダ名.jsonが存在するか）
    if json_path.is_file():
        ...
        # バッチ処理モード: jsonが見つからない場合は、配下の全フォルダを検索
        print(f"🚀 バッチ処理モード: {input_dir} 以下の全対象フォルダを処理します")
        processed_count = 0
        ...
        for subdir in input_dir.iterdir():
            ...
            sub_json_path = subdir / f"{subdir.name}.json"
            if not sub_json_path.is_file():
                continue # 対象外フォルダをスキップ
                
            sub_image_dir = subdir / f"{subdir.name}_images"
            if not sub_image_dir.is_dir():
                sub_image_dir = None
                
            sub_deck_name = f"QB::{subdir.name}"
            sub_output_path = input_dir / f"{subdir.name}.apkg"
            
            print(f"\n" + "="*50)
            print(f"🔹 処理開始: {subdir.name}")
            
            try:
                convert_to_anki(
                    json_path=sub_json_path,
                    image_dir=sub_image_dir,
                    deck_name=sub_deck_name,
                    output_path=sub_output_path,
                    tags=tags, # タグは共通
                )
                processed_count += 1
            except Exception as e:
                print(f"❌ 処理エラー ({subdir.name}): {e}", file=sys.stderr)
                
        print(f"\n" + "="*50)
        print(f"🎉 バッチ処理完了: 計 {processed_count} 個のデッキを出力しました")
